...
> "No active applications to follow up on. Apply to some roles first with `/career` and come back when they're aging."
...
- Suggest the user run `/career contacto {company}` to find the right person first
...
> - Trying a different contact via `/career contacto`
...
**To:** {email or "No contact found — run `/career contacto` first"}
