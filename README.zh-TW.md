...
> **重要：這不是廣撒網的工具。** career 是一個篩選器 — 它幫助你從數百份職缺中找出真正值得投入的少數機會。本系統強烈建議不要應徵評分低於 4.0/5 的職缺。你的時間很寶貴，招募人員的時間也是。送出前務必仔細審閱。

career 具有代理能力：Claude Code 透過 Playwright 瀏覽求職頁面，藉由推理你的履歷與職缺描述的契合度（而非關鍵字比對）進行評估，並針對每份職缺調整你的履歷。
...
這個系統由一位親身使用它評估超過 740 份職缺、生成超過 100 份客製化履歷、並成功獲得 Head of Applied AI 職位的人所打造。[閱讀完整案例研究](https://santifer.io/career-system)。
...
career 是一個具有多種模式的單一斜線指令：

```
/career                → 顯示所有可用指令
/career {貼上職缺描述}  → 完整自動管道（評估 + PDF + 追蹤）
/career scan           → 掃描平台尋找新職缺
/career pdf            → 生成 ATS 最佳化履歷
/career batch          → 批次評估多份職缺
/career tracker        → 查看應徵狀態
/career apply          → AI 協助填寫應徵表單
/career pipeline       → 處理待辦 URL
/career contacto       → LinkedIn 外寄訊息
/career deep           → 深度公司研究
/career training       → 評估課程/證照
/career project        → 評估作品集專案
```

或者直接貼上職缺 URL 或描述 — career 會自動偵測並執行完整管道。
...
我是 Santiago — Head of Applied AI，前創業者（創建並出售了一家至今仍以我名字營運的公司）。我打造 career 是為了管理自己的求職過程，並成功用它找到了現在這份工作。

個人作品集與其他開源專案 → [santifer.io](https://santifer.io)

☕ 如果 career 對你的求職有所幫助，歡迎[請我喝杯咖啡](https://buymeacoffee.com/santifer)。
...
**career 是一個本地端開源工具 — 並非託管服務。** 使用本軟體即表示你確認：
...
<a href="https://github.com/santifer/career/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=santifer/career" />
</a>

使用 career 找到工作了嗎？[分享你的故事！](https://github.com/santifer/career/issues/new?template=i-got-hired.yml)
